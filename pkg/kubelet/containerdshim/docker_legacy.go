...
// detail in containerdshim. However, we need these labels for legacy container (containers created by
...
// NOTE that we can't handle the following containerdshim internal labels, so they will be empty:
...
// convertLegacyNameAndLabels converts legacy name and labels into containerdshim name and labels.
...
func convertLegacyNameAndLabels(names []string, labels map[string]string) ([]string, map[string]string, error) {
	...
	// Generate new containerdshim name.
	...
	for k, v := range labels {
		if legacyAnnotations.Has(k) {
			// Add annotation prefix for all legacy labels which should be annotations in containerdshim.
